...
var gKey = 'AIzaSyA1tYTBr0Je-1N6AQ-Mmv87Ed9dYdoAmnQ';
var gTable = '1vgzsjejB5NZo7X8D0Xp4-ygLOMvw1IOpjQN8G3J_';
...
function sconvertJSON( serialObj ) {
	...
	{ /*check for NaN values*/
	    if (isNaN(s.rows[i][j]) && ((s.columns[j].toLowerCase().search("range") != -1)||(s.columns[j].toLowerCase().search("power") != -1)||(s.columns[j].toLowerCase().search("cooldown") != -1)||(s.columns[j].toLowerCase().search("_luck") != -1)||(s.columns[j].toLowerCase().search("_hp") != -1)||(s.columns[j].toLowerCase().search("_atk") != -1)||(s.columns[j].toLowerCase().search("_spd") != -1)||(s.columns[j].toLowerCase().search("_id") != -1))) {
